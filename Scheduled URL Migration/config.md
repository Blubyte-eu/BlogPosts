...
add responder policy RS_POL_SCHEDULE_REDIRECT_URL "HTTP.REQ.HOSTNAME.SERVER.EQ(\"old.domain.com\")&&HTTP.REQ.URL.STARTSWITH_ANY(\"PS_SCHEDULE_REDIRECT_URLS\")&&HTTP.REQ.URL.HTTP_URL_SAFE.REGEX_SELECT(re!^\\/[a-zA-Z0-9-_]+\\/[a-zA-Z0-9-_]+!).MAP_STRING(\"SM_REDIRECTION_SCHEDULE\").TYPECAST_TIME_T.LE(SYS.TIME)" NOOP -logAction LOG_MIGRATION_SCHEDULE
```
